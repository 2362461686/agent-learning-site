...
import PostProgressTracker from '../../../components/PostProgressTracker';
import ResourceSection from '../../../components/ResourceSection'; // ✅ Phase2: 学习资源展示
...
async function getPostData(slug: string) {
  ...
  return {
    ...
    // ✅ Phase2: 学习视频和链接
    videos: data.videos && Array.isArray(data.videos) ? data.videos : [],
    ...
    <div className="min-h-screen relative pb-20">
      ...
      <PageTransition>
        <main className="w-[95%] md:w-[90%] max-w-6xl mx-auto mt-24 md:mt-28 flex flex-col lg:flex-row gap-6 md:gap-8 relative z-10">
          ...
          <article className="flex-1 bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 dark:border-white/10 overflow-hidden transition-colors duration-700">
            <div className="w-full aspect-video bg-slate-200 dark:bg-slate-700 relative group">
              {postData.cover ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-indigo-500/60 via-purple-500/60 to-cyan-500/60 flex items-center justify-center">
                  <span className="text-white/40 text-6xl font-black">{postData.title?.charAt(0) || "A"}</span>
                </div>
              )}
            </div>
          ...
          <aside className="w-full lg:w-[320px] flex flex-col gap-6 flex-shrink-0 lg:sticky lg:top-28 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto custom-scrollbar">
            ...
            {/* ✅ Phase2: 学习资源（视频 + 链接） */}
            <ResourceSection videos={postData.videos} links={postData.links} />
          </aside>
        </main>
        <PostProgressTracker slug={resolvedParams.slug} />
      </PageTransition>
    </div>
  );
}